...
"""WSL2-friendly variant of pause.py.

The sandbox MicroVM boots quickly, but the Jupyter kernel gateway on :49999
can take a few seconds to accept connections. This script polls the gateway
/health endpoint before the first run_code() call, mirroring the behaviour of
exec_code_wsl.py.
"""
...
parser = argparse.ArgumentParser(description="Cube Sandbox Pause/Resume TUI Demo (WSL2)")
parser.add_argument(
...
console.print(
    Panel(
        "[bold]Cube Sandbox · Pause / Resume Demo (WSL2)[/]",
        box=box.DOUBLE,
        style=PAL["accent"],
        expand=True,
    )
